<template>
  <div>
    <my-breadcrumbs title="创新创业" />
    <p style="text-align: center; margin: 20px" v-if="isNull">暂无数据</p>
    <div class="innovationAndEntrepreneurship-class">
      <template
        v-if="
          cardList.length === 0 ||
      ...
      <my-card
        ...
        :from="'innovationAndEntrepreneurship_' + current_page"
      />
      <my-paging
        v-if="!isNull"
...
export default {
  name: "innovationAndEntrepreneurship",
  props: {
    page: String,
  },
  components: {
...
<style scoped>
.innovationAndEntrepreneurship-class {
  width: 100%;
  display: flex;
