<template>
  <div>
    <template v-if="statue">
      ...
      <div class="excellentCurriculumDesignU-class">
        ...
        <my-detail
          ...
          :current="i"
          :list="idList"
          :from="'/gallery/excellentCurriculumDesignU/' + $route.params.id"
        />
        <my-paging
    ...
    <template v-else>
      <my-breadcrumbs title="优秀课程设计" subTitle="本科阶段" />
...
export default {
  ...
  methods: {
    ...
    goBack() {
      this.statue = false;
      this.$router.push(`/gallery/excellentCurriculumDesignU`);
    },
    toDesign(data) {
      this.$router.push(`/gallery/excellentCurriculumDesignU/${data.id}`);
